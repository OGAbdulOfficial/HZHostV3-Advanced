...
async def add_user(user_id: int, username: str):
    ...
    Also handles initial approval status.
    """
    # Admins are approved by default
    is_approved = not config.Bot.REQUIRE_APPROVAL or user_id in config.Bot.ADMIN_IDS
...
async def update_user_approval(user_id: int, status: bool):
    """Updates the approval status for a user."""
    await users_collection.update_one(
        {'_id': user_id},
